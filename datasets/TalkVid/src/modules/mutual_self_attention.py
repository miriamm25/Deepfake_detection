    ...
    def update(
        ...
        if self.reference_attn:
            ...
            for r, w in zip(reader_attn_modules, writer_attn_modules):
                if do_classifier_free_guidance:
                    r.bank = [torch.cat([torch.zeros_like(v), v]).to(device=v.device, dtype=dtype) for v in w.bank]
                elif do_unconditional_forward:
                    r.bank = [torch.zeros_like(v).to(device=v.device, dtype=dtype) for v in w.bank]
                else:
                    r.bank = [v.clone().to(device=v.device, dtype=dtype) for v in w.bank]
