...
INPUT_DIR="$DATA_PREFIX/$DATASET/videos"   # 数据集的输入视频目录
OUTPUT_DIR="$DATA_PREFIX/$DATASET/videos-crop"   # 输出目录，用于存储提取的面部信息
LOG_DIR="$ROOT/logs/face_crop"  # 日志目录
...
do
    echo "GPU $i → 日志: $LOG_DIR/face_crop_$i.log"
    CUDA_VISIBLE_DEVICES=$i python face_crop.py \
      --input_dir "$INPUT_DIR" \
      ...
      --num_gpus $NUM_GPUS \
      --shard $SHARD \
      > $LOG_DIR/face_crop_$i.log 2>&1 &  # 每张卡输出日志
done

# ==== 等待所有任务完成 ====
wait
echo "✅ face-crop 所有任务执行完成"
