...
def plan_clips_for_row(row: pd.Series, policy: dict, rng: random.Random) -> List[dict]:
    # DF40 streams manifest already includes split + duration_bucket
    vid = row["video_id"]
    split = row["split"]
...
def main():
    ...
    ap.add_argument("--manifest", required=True, help="Path to DF40 streams manifest CSV")
    ap.add_argument("--out_manifest", default="manifests/df40_clips_manifest.csv")
    ap.add_argument("--out_dir", default="reports/df40_clip_plan")
    ...
    summary = {
        ...
        "clips_by_split": clips_df.groupby("split")["clip_id"].count().to_dict() if len(clips_df) else {},
        "clips_by_bucket": clips_df.groupby("duration_bucket")["clip_id"].count().to_dict() if len(clips_df) else {},
        "clips_by_orientation": clips_df.groupby("orientation")["clip_id"].count().to_dict() if len(clips_df) else {},
        "clips_by_fps_bucket": clips_df.groupby("fps_bucket")["clip_id"].count().to_dict() if len(clips_df) else {},
        "clips_by_audio_group": clips_df.groupby("audio_group")["clip_id"].count().to_dict() if len(clips_df) else {},
